#!/usr/bin/env python3
"""End-to-end from wherever this checkout came from, not the public copy.
...
somewhere unrelated, because nothing here is faked: a real clone -- here of this checkout's own
origin, mirrored locally by the harness so the agent needs no credential
of its own -- a real bundle-setup dialogue resolved
autonomously, a real document, real commits, a real push.

Starting from the source rather than the published copy matters: the
source carries files the published one does not, and a difference
between this case and e2e-github is a finding about what publishing
strips.
"""
